...
is_repo_name() {
  local name="$1"
  [[ -d "$PPM_DATA_HOME/$name/$PPM_ASSET_DIR" ]]
}

# Collect assets, optionally filtered to specific repos
# Usage: collect_assets [repo1 repo2 ...]
...
collect_assets() {
  ...
  for repo_name in ${filter_repos[@]+"${filter_repos[@]}"}; do
    local repo_path="$PPM_DATA_HOME/$repo_name/$PPM_ASSET_DIR"
...
expand_packages() {
  ...
  for arg in "$@"; do
    if [[ "$arg" == */ ]] && is_repo_name "${arg%/}"; then
      arg="${arg%/}"
      collect_assets "$arg"
      [[ ${#PACKAGES[@]} -eq 0 ]] && { echo "Error: No ${PPM_ASSET_LABEL}s found in repo '$arg'"; exit 1; }
      if ! $force; then
